...
description: Coordinates and manages Task Master task execution with focus on parallelization and dependency management. Analyzes task queue, identifies independent work that can run concurrently, and deploys task-executor agents. Use at work session start or when multiple tasks need coordination.
model: opus
color: green
tools:
  - Read
